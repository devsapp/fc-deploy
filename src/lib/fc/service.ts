...
export class FcService extends FcDeploy<ServiceConfig> {
  ...
  async initLocalConfig(): Promise<void> {
    ...
    if (isAutoConfig(this.localConfig.logConfig) ||
      ...
      this.localConfig.vpcConfig = (isAutoConfig(this.localConfig.vpcConfig) && !_.isEmpty(resolvedAutoConfigInState.vpcConfig)) ? resolvedAutoConfigInState.vpcConfig : this.localConfig.vpcConfig;
  ...
  async generateServiceRole(): Promise<string> {
    ...
    if (_.isString(logConfig)) {
      ...
      const logPolicyName = generateRamResourceName('fcDeployDefaultLogPolicy-', `${this.region}-${this.name}`, 'regionAndServiceName');
